--[[
    i took this from https://github.com/notcake/glib/blob/master/lua/glib/bitconverter.lua
    it is licensed under GPLv3
    as long as this entire project is GPLv3 too then that shouldn't be a problem
    please don't sue me
]]
...
-- Integers
function bit_conv.UInt8ToUInt8s (n)
	return n
end

function bit_conv.UInt16ToUInt8s (n)
	return             n        % 256,
	       math_floor (n / 256) % 256
end

function bit_conv.UInt32ToUInt8s (n)
	return             n             % 256,
	       math_floor (n /      256) % 256,
...
function bit_conv.UInt64ToUInt8s (n)
	return             n                      % 256,
	       math_floor (n /               256) % 256,
	       math_floor (n /             65536) % 256,
	       math_floor (n /          16777216) % 256,
...
function bit_conv.UInt8sToUInt8(uint80)
	return uint80
end

function bit_conv.UInt8sToUInt16 (uint80, uint81)
	return uint80 +
	       uint81 * 256
end

function bit_conv.UInt8sToUInt32 (uint80, uint81, uint82, uint83)
	return uint80 +
	       uint81 * 256 +
	       uint82 * 65536 +
	       uint83 * 16777216
...
function bit_conv.UInt8sToUInt64 (uint80, uint81, uint82, uint83, uint84, uint85, uint86, uint87)
	return uint80 +
	       uint81 * 256 +
	       uint82 * 65536 +
	       uint83 * 16777216 +
...
function bit_conv.Int8ToUInt8s (n)
	if n < 0 then n = n + 256 end
	return bit_conv.UInt8ToUInt8s (n)
end

function bit_conv.Int16ToUInt8s (n)
	if n < 0 then n = n + 65536 end
	return bit_conv.UInt16ToUInt8s (n)
end

function bit_conv.Int32ToUInt8s (n)
	if n < 0 then n = n + 4294967296 end
	return bit_conv.UInt32ToUInt8s (n)
end

function bit_conv.Int64ToUInt8s (n)
	local uint80, uint81, uint82, uint83 = bit_conv.UInt32ToUInt8s (n % 4294967296)
	local uint84, uint85, uint86, uint87 = bit_conv.Int32ToUInt8s (math_floor (n / 4294967296))
	return uint80, uint81, uint82, uint83, uint84, uint85, uint86, uint87
end

function bit_conv.UInt8sToInt8 (uint80)
	local n = bit_conv.UInt8sToUInt8 (uint80)
	if n >= 128 then n = n - 256 end
	return n
end

function bit_conv.UInt8sToInt16 (uint80, uint81)
	local n = bit_conv.UInt8sToUInt16 (uint80, uint81)
	if n >= 32768 then n = n - 65536 end
	return n
end

function bit_conv.UInt8sToInt32 (uint80, uint81, uint82, uint83)
	local n = bit_conv.UInt8sToUInt32 (uint80, uint81, uint82, uint83)
	if n >= 2147483648 then n = n - 4294967296 end
	return n
end

function bit_conv.UInt8sToInt64 (uint80, uint81, uint82, uint83, uint84, uint85, uint86, uint87)
	local low  = bit_conv.UInt8sToUInt32 (uint80, uint81, uint82, uint83)
	local high = bit_conv.UInt8sToInt32 (uint84, uint85, uint86, uint87)
	return low + high * 4294967296
end

-- IEEE floating point numbers
function bit_conv.FloatToUInt32 (f)
...
function bit_conv.DoubleToUInt32s (f)
	-- 1 / f is needed to check for -0
	local high = 0
	local low  = 0
	if f < 0 or 1 / f < 0 then
...
function bit_conv.UInt32ToFloat (n)
	-- 1 sign bit
...
function bit_conv.UInt32sToDouble (low, high)
	-- 1 sign bit
